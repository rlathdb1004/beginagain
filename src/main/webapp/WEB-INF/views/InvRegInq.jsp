		...
		<div class="taToolbarField">
			<select class="taSelect" name="searchType"><option value=""
				...
				<option value="itemName"
					${invRegInqSearchDTO.searchType eq 'itemName' ? 'selected' : ''}>품목명</option></select>
		</div>
		<div class="taToolbarField">
...
<c:if test="${not empty invRegInqDTO}">
	<div class="taTableShell" style="padding: 20px; margin-top: 20px;">
		...
		<p>
			<strong>재고번호 :</strong> ${invRegInqDTO.inventoryId}
		</p>
		<p>
			<strong>품목코드 :</strong> ${invRegInqDTO.itemCode}
		</p>
		<p>
			<strong>품목명 :</strong> ${invRegInqDTO.itemName}
		</p>
		<p>
			<strong>현재재고 :</strong> ${invRegInqDTO.qtyOnHand}
		</p>
		<p>
			<strong>안전재고 :</strong> ${invRegInqDTO.safetyStock}
		</p>
		<p>
			<strong>단위 :</strong> ${invRegInqDTO.unit}
		</p>
		<p>
			<strong>비고 :</strong> ${invRegInqDTO.remark}
		</p>
	</div>
</c:if>