...
<div class="taPageActions">
    <button type="button" class="taOpenModal taBtn taBtnPrimary" data-modal-target="invRegisterModal">등록</button>
    <button type="submit" form="invDeleteForm" class="taBtn taBtnOutline">선택 삭제</button>
</div>
...
<div id="invRegisterModal" class="taModal" hidden aria-hidden="true">
    <div class="taModalDialog">
        <div class="taModalHeader">
            ...
            <button type="button" class="taModalClose" data-modal-target="invRegisterModal">×</button>
        </div>
        <form method="post" action="${pageContext.request.contextPath}/invRegInq">
            ...
            <div class="taModalBody taModalGrid">
                ...
                <div class="form-row"><label>현재재고</label><input type="number" step="0.001" name="qtyOnHand" required></div>
                <div class="form-row"><label>안전재고</label><input type="number" step="0.001" name="safetyStock" required></div>
...
<form id="paSearchForm" method="post" action="${pageContext.request.contextPath}/invRegInq">
    ...
    <div class="taToolbarRow">
        <div class="taToolbarField">
            <select class="taSelect" name="searchType">
                <option value="" ${empty invRegInqSearchDTO.searchType ? 'selected' : ''}>전체</option>
                <option value="itemCode" ${invRegInqSearchDTO.searchType eq 'itemCode' ? 'selected' : ''}>품목코드</option>
                <option value="itemName" ${invRegInqSearchDTO.searchType eq 'itemName' ? 'selected' : ''}>품목명</option>
            </select>
        </div>
        <div class="taToolbarField">
            <div class="taSearchBox">
                <input type="date" class="taSearchInput" name="startDate" value="${invRegInqSearchDTO.startDate}">
                <input type="date" class="taSearchInput" name="endDate" value="${invRegInqSearchDTO.endDate}">
            </div>
        </div>
        <div class="taToolbarField taToolbarFieldGrow">
            <div class="taSearchBox">
                <input type="text" class="taSearchInput" name="keyword" placeholder="검색키워드" value="${invRegInqSearchDTO.keyword}">
...
<form id="invDeleteForm" method="post" action="${pageContext.request.contextPath}/invRegInq">
                ...
                <tbody>
                    <c:forEach var="dto" items="${invRegInqList}">
                        <tr class="taTableBodyRow">
                            <td class="taTableBodyCell taColFit"><input type="checkbox" class="taCheckInput" name="inventoryIds" value="${dto.inventoryId}"></td>
                            ...
                            <td class="taTableBodyCell taColAction taLastCol"><a class="taLinkAnchor" href="${pageContext.request.contextPath}/invRegInq?inventoryId=${dto.inventoryId}">상세보기</a></td>
                        </tr>
                    </c:forEach>
                    <c:if test="${empty invRegInqList}">
                        <tr class="taTableBodyRow"><td class="taTableBodyCell taLastCol" colspan="9" style="text-align:center;">조회된 데이터가 없습니다.</td></tr>
                    </c:if>
                </tbody>
            </table>
        </div>
