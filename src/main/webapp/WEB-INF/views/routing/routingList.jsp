...
<form method="get" action="${pageContext.request.contextPath}/routing/list" class="taLocalSearchForm">
    <div class="taToolbarRow">
        <div class="taToolbarField taToolbarFieldGrow taSpan2">
            <label style="display:block; margin-bottom:8px; font-weight:600;">완제품 선택</label>
...
<form id="deleteForm" action="${pageContext.request.contextPath}/routing/delete" method="post">
        ...
        <div class="taTableScroll">
            <table class="taMesTable taRoutingTable" id="routingTable">
                ...
                <tbody>
                    <c:choose>
                        <c:when test="${empty selectedItemId}">
                            <tr class="taTableBodyRow"><td class="taTableBodyCell taLastCol" colspan="5" style="text-align:center;">완제품을 선택하면 라우팅이 조회됩니다.</td></tr>
                        </c:when>
                        <c:when test="${not empty routingList}">
                            <c:forEach var="routing" items="${routingList}">
                                <tr class="taTableBodyRow">
                                    <td class="taTableBodyCell taCheckCell"><input type="checkbox" name="routingId" value="${routing.routingId}" class="taCheckInput"></td>
                                    <td class="taTableBodyCell taColFit">${routing.processSeq}</td>
                                    <td class="taTableBodyCell taColGrow">${routing.processName}</td>
                                    <td class="taTableBodyCell taColGrow"><c:choose><c:when test="${not empty routing.equipmentName}">${routing.equipmentName}</c:when><c:otherwise>-</c:otherwise></c:choose></td>
                                    <td class="taTableBodyCell taColAction taLastCol"><a class="taLinkAnchor" href="${pageContext.request.contextPath}/routing/detail?routingId=${routing.routingId}">상세보기</a></td>
                                </tr>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
                            <tr class="taTableBodyRow"><td class="taTableBodyCell taLastCol" colspan="5" style="text-align:center;">해당 품목의 라우팅 데이터가 없습니다.</td></tr>
                        </c:otherwise>
                    </c:choose>
                </tbody>
            </table>
...
<div class="taModal" id="registerModal" hidden aria-hidden="true">
    <div class="taModalDialog modal-lg">
        ...
        <form action="${pageContext.request.contextPath}/routing/register" method="post">
            <input type="hidden" name="itemId" value="${selectedItemId}">
            <div class="taModalBody taModalGrid">
                <div class="form-row full">
                    ...
                    <div class="taReadonlyText">
                        <c:forEach var="item" items="${itemList}">
                            <c:if test="${item.itemId == selectedItemId}">${item.itemCode} - ${item.itemName}</c:if>
                        </c:forEach>
                    </div>
                </div>
                <div class="form-row">
                    ...
                    <select name="processId" required>
                        ...
                        <c:forEach var="process" items="${processList}">
                            <option value="${process.processId}">${process.processCode} - ${process.processName}</option>
                        </c:forEach>
                    </select>
                </div>
                <div class="form-row">
                    ...
                    <select name="equipmentId" required>
                        ...
                        <c:forEach var="equipment" items="${equipmentList}">
                            <option value="${equipment.equipmentId}">${equipment.equipmentCode} - ${equipment.equipmentName}</option>
                        </c:forEach>
                    </select>
                </div>
                <div class="form-row">
