...
<div class="taFormShell taEmptyState"><p>조회된 자재 검사 정보가 없습니다.</p><div class="taPageActions" style="justify-content:center; margin-top:16px;"><a class="taBtn taBtnOutline" href="${pageContext.request.contextPath}/matInspRegInq" style="text-decoration:none;">목록</a></div></div>
</c:when>
<c:otherwise>
<form id="matInspUpdateForm" action="${pageContext.request.contextPath}/matInspRegInq" method="post">
<input type="hidden" name="cmd" value="update">
<input type="hidden" name="materialInspectionId" value="${matInspRegInqDTO.materialInspectionId}">
...
<tr><th class="taFormLabel">단위</th><td class="taFormValue"><span class="taReadonlyText">${matInspRegInqDTO.unit}</span></td></tr>
<tr><th class="taFormLabel">검사자</th><td class="taFormValue"><span class="taReadonlyText">${matInspRegInqDTO.empName}</span></td></tr>
<tr><th class="taFormLabel">검사수량</th><td class="taFormValue"><input class="taFormInput taEditableField" type="number" step="0.001" min="0.001" name="inspectQty" value="${matInspRegInqDTO.inspectQty}"></td></tr>
...
<tr><th class="taFormLabel">판정</th><td class="taFormValue"><select class="taFormInput taEditableField" name="result"><option value="합격" ${matInspRegInqDTO.result eq '합격' ? 'selected' : ''}>합격</option><option value="부분합격" ${matInspRegInqDTO.result eq '부분합격' ? 'selected' : ''}>부분합격</option><option value="불합격" ${matInspRegInqDTO.result eq '불합격' ? 'selected' : ''}>불합격</option></select></td></tr>
...
<tr><th class="taFormLabel">생성일</th><td class="taFormValue"><span class="taReadonlyText">${matInspRegInqDTO.createdAt}</span></td></tr>
<tr><th class="taFormLabel">수정일</th><td class="taFormValue"><span class="taReadonlyText">${matInspRegInqDTO.updatedAt}</span></td></tr>
</table></div></form>
