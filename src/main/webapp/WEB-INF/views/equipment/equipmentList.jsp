...
<form id="deleteForm" action="${pageContext.request.contextPath}/equipment/delete" method="post">
  <div class="taTableShell"><div class="taTableScroll"><table class="taMesTable" id="equipmentTable">
    <thead><tr>
    ...
    <tbody>
      <c:choose><c:when test="${not empty equipmentList}">
        <c:forEach var="equipment" items="${equipmentList}"><tr class="taTableBodyRow">
          <td class="taTableBodyCell taCheckCell"><input type="checkbox" name="equipmentId" value="${equipment.equipmentId}" class="taCheckInput"></td>
          <td class="taTableBodyCell taColFit">${equipment.equipmentId}</td>
          <td class="taTableBodyCell taColFit" data-search-key="equipmentCode">${equipment.equipmentCode}</td>
          <td class="taTableBodyCell taColGrow" data-search-key="equipmentName">${equipment.equipmentName}</td>
          ...
          <td class="taTableBodyCell taColFit" data-search-key="vendorName">${equipment.vendorName}</td>
          <td class="taTableBodyCell taColFit">${equipment.equipmentPrice}</td>
          <td class="taTableBodyCell taColAction taLastCol"><a class="taLinkAnchor" href="${pageContext.request.contextPath}/equipment/detail?equipmentId=${equipment.equipmentId}">상세보기</a></td>
        </tr></c:forEach>
      </c:when><c:otherwise><tr class="taTableBodyRow"><td class="taTableBodyCell taLastCol" colspan="10" style="text-align:center;">조회된 설비가 없습니다.</td></tr></c:otherwise></c:choose>
    </tbody></table></div></div>
</form>
<div class="taModal" id="registerModal" hidden aria-hidden="true"><div class="taModalDialog modal-lg"><div class="taModalHeader"><h3 class="taModalTitle">설비 등록</h3><button type="button" class="taModalClose">&times;</button></div>
<form action="${pageContext.request.contextPath}/equipment/register" method="post"><div class="taModalBody taModalGrid">
...
</div><div class="taModalFooter"><button type="button" class="taBtn taBtnOutline taModalClose">취소</button><button type="submit" class="taBtn taBtnPrimary">등록</button></div></form></div></div>
