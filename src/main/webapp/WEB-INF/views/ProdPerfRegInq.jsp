		...
		<div class="taToolbarRow">
			<div class="taToolbarField taToolbarSpan2">
				<select
					...
					name="searchType">
					<option value="" hidden ${empty param.searchType ? "selected" : ""}>전체 / 작업지시번호 ...</option>
			...
			<div class="taToolbarField taToolbarFieldGrow taToolbarSpan6">
				<div class="taSearchBox">
					...
					<button type="button" class="taBtn taBtnOutline taSearchReset" onclick="location.href='${pageContext.request.contextPath}/prodperf'">초기화</button>
				</div>
			</div>
		</div>
	</form>
				...
				<table class="taMesTable">
					<thead>
						<tr>
							...
							<th class="taTableHeadCell taColFit">손실량</th>
					...
					<tbody>
						<c:forEach var="dto" items="${list}">
							<tr class="taTableBodyRow">
								<td class="taTableBodyCell taColFit"><input type="checkbox" name="seqNO" value="${dto.seqNO}" class="taCheckInput rowCheck"></td>
								...
								<td class="taTableBodyCell taColFit">${dto.lossQty}</td>
						...
						<c:if test="${empty list}">
							<tr class="taTableBodyRow">
								<td class="taTableBodyCell taLastCol" colspan="10" style="text-align: center;">조회된 데이터가 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
				</table>
					...
					<select name="workOrderId" id="registerWorkOrderId" required>
						...
						<c:forEach var="wo" items="${workOrderOptions}">
							<option value="${wo.workOrderId}"
								...
								data-remaining-qty="${wo.remainingQty}">${wo.workOrderNo}/ ${wo.itemName} / ${wo.lineCode}</option>
						</c:forEach>
					</select>
				</div>
...
document.addEventListener("DOMContentLoaded", function() {
	...
	const fields = {
		...
		currentProduced: document.getElementById("registerCurrentProduced"),
	...
	function syncRegisterWorkOrderMeta() {
		...
		fields.workQty.value = opt ? (opt.dataset.workQty || "") : "";
		fields.itemCode.value = opt ? (opt.dataset.itemCode || "") : "";
		fields.itemName.value = opt ? (opt.dataset.itemName || "") : "";
		fields.currentProduced.value = opt ? (opt.dataset.currentProduced || "") : "";
		fields.currentLoss.value = opt ? (opt.dataset.currentLoss || "") : "";
