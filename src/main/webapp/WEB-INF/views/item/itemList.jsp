    ...
    <div class="taToolbarRow">
        <div class="taToolbarField taToolbarSpan3">
            <select class="taSelect taAutoSelectColor ${empty searchType ? 'taSelectPlaceholder' : ''}" name="searchType">
                <option value="" ${empty searchType ? 'selected' : ''}>전체 / 품목코드 ...</option>
        ...
        <div class="taToolbarField taToolbarFieldGrow taToolbarSpan9">
            <div class="taSearchBox">
                ...
                <button type="button" class="taBtn taBtnOutline taSearchReset" onclick="location.href='${pageContext.request.contextPath}/item/list'">초기화</button>
            </div>
        </div>
    </div>
</form>

<form id="deleteForm" method="post" action="${pageContext.request.contextPath}/item/delete">
            ...
            <table class="taMesTable">
                ...
                <tbody>
                    <c:choose>
                            ...
                            <c:forEach var="item" items="${list}">
                                <tr class="taTableBodyRow">
                                    <td class="taTableBodyCell taCheckCell">
                                        <c:if test="${isMesAdmin}">
                                            <input type="checkbox" name="itemId" value="${item.itemId}" class="taCheckInput">
                                        </c:if>
                                    </td>
                                    ...
                                    <td class="taTableBodyCell taColAction taLastCol">
                                        <button type="button" class="taLinkButton" onclick="location.href='${pageContext.request.contextPath}/item/detail?itemId=${item.itemId}'">상세보기</button>
                                    </td>
                                </tr>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
                            <tr class="taTableBodyRow">
                                <td class="taTableBodyCell taLastCol" colspan="6">조회된 품목이 없습니다.</td>
                            </tr>
                        </c:otherwise>
                    </c:choose>
                </tbody>
...
<c:if test="${isMesAdmin}">
    <div class="taModal" id="registerModal" hidden aria-hidden="true">
        <div class="taModalDialog">
            ...
            <form method="post" action="${pageContext.request.contextPath}/item/register">
...
(function() {
    ...
    if (checkAll) {
        checkAll.addEventListener('change', function() {
            document.querySelectorAll("input[name='itemId']").forEach(function(cb) {
