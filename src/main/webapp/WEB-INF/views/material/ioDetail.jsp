...
<c:set var="canEdit" value="${isManager and ioRegInqDTO.status ne '완료'}" />
...
<c:choose>
	...
	<c:otherwise>
		<form id="ioUpdateForm"
            ...
            <input type="hidden" name="status" value="${empty ioRegInqDTO.status ? "완료" : ioRegInqDTO.status}">
			...
			<div class="taFormShell">
				<table class="taFormTable">
					...
					<tr>
						...
						<td class="taFormValue"><span class="taReadonlyText">${ioRegInqDTO.itemType}</span></td>
					</tr>
					<tr>
						<th class="taFormLabel">단위</th>
						<td class="taFormValue"><span class="taReadonlyText">${ioRegInqDTO.unit}</span></td>
					</tr>
					<tr>
						<th class="taFormLabel">현재고</th>
						<td class="taFormValue"><span class="taReadonlyText">${ioRegInqDTO.currentStock}</span></td>
					</tr>
					<tr>
						<th class="taFormLabel">입출고구분</th>
						<td class="taFormValue"><select
							class="taFormInput taEditableField" name="inoutType">
								<option value="입고"
									${ioRegInqDTO.inoutType eq '입고' ? 'selected' : ''}>입고</option>
								<option value="출고"
									${ioRegInqDTO.inoutType eq '출고' ? 'selected' : ''}>출고</option>
						</select></td>
					</tr>
					<tr>
						...
						<td class="taFormValue"><input
							...
							min="0.001" name="qty" value="${ioRegInqDTO.qty}"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="taFormValue"><span class="taReadonlyText">${ioRegInqDTO.createdAt}</span></td>
					</tr>
					<tr>
						<th class="taFormLabel">수정일</th>
						<td class="taFormValue"><span class="taReadonlyText">${ioRegInqDTO.updatedAt}</span></td>
					</tr>
				</table>
			</div>
		</form>
