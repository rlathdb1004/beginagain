...
<div class="prodplan-page">
	...
	<div class="taPageActions">
		<button type="button" class="taOpenModal taBtn taBtnPrimary" data-modal-target="prodPlanRegisterModal">등록</button>
	...
	<div id="prodPlanRegisterModal" class="taModal" hidden aria-hidden="true">
		<div class="taModalDialog">
			<div class="taModalHeader">
				...
				<button type="button" class="taModalClose" data-modal-target="prodPlanRegisterModal">×</button>
			</div>
			<form method="post" action="${pageContext.request.contextPath}/prodplan">
				...
				<div class="taModalBody taModalGrid">
					<div class="form-row full">
						<label>품목</label>
						<select name="planCode" class="taFormInput" required>
							<option value="">선택</option>
							<c:forEach var="item" items="${itemOptions}">
								<option value="${item.planCode}">${item.planCode} / ${item.planName}</option>
							</c:forEach>
						</select>
					</div>
					<div class="form-row"><label>계획일자</label><input type="date" name="planDate" required></div>
					<div class="form-row"><label>계획수량</label><input type="number" name="planAmount" min="0" required></div>
					<div class="form-row"><label>라인</label><input type="text" name="planLine" placeholder="예: LN-A" required></div>
					<div class="form-row"><label>상태</label>
						<select name="status" class="taFormInput">
					...
					<div class="form-row full"><label>비고</label><textarea name="memo"></textarea></div>
				</div>
	...
	<form id="paSearchForm" method="get" action="${pageContext.request.contextPath}/prodplan">
		<input type="hidden" name="searched" value="Y"> <input type="hidden" name="page" id="paPage" value="${paCurrentPage}">
		<div class="taToolbarRow">
			<div class="taToolbarField">
				<div class="taSearchBox">
					<input type="date" class="taSearchInput" name="startDate" value="${param.startDate}"> <input type="date" class="taSearchInput" name="endDate" value="${param.endDate}">
				</div>
			</div>
			<div class="taToolbarField">
			...
			<div class="taToolbarField taToolbarFieldGrow">
				<div class="taSearchBox">
					<input type="text" class="taSearchInput" name="keyword" placeholder="생산계획번호 / 품목코드 / 품목명 / 라인 검색" value="${param.keyword}">
					<button type="submit" class="taSearchBtn" aria-label="검색"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"></circle><path d="M20 20L16.65 16.65"></path></svg></button>
				</div>
			</div>
		</div>
	</form>
					...
					<tbody>
						<c:forEach var="dto" items="${list}">
							<tr class="taTableBodyRow">
								...
								<td class="taTableBodyCell taColAction taLastCol"><a class="taLinkAnchor" href="${pageContext.request.contextPath}/prodplan?seqNO=${dto.seqNO}">상세보기</a></td>
							</tr>
						</c:forEach>
...
document.addEventListener("DOMContentLoaded", function() {
	...
	if (checkAll) {
		checkAll.addEventListener("change", function() {
			document.querySelectorAll(".rowCheck").forEach(function(checkbox) { checkbox.checked = checkAll.checked; });
		});
	}
	window.handleDeleteButton = function() {
		const deleteCols = document.querySelectorAll(".delete-col");
		const rowChecks = document.querySelectorAll(".rowCheck");
		if (rowChecks.length === 0) { alert("삭제할 데이터가 없습니다."); return; }
		...
		if (confirm("선택한 생산계획을 삭제하시겠습니까?")) deleteForm.submit();
	};
});
</script>
